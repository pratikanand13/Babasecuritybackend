    ...
    router.get('/apiDiscovery', auth, webCrawling, async (req, res) => {
        try {
            ...
            // Prepare the apiName array with API URLs
            const apiName = data.map(api => ({
                api: api.url,  // API URL
            }));
    
            // Find the document associated with the organisationId
            const api = await dashboardSchema.findOne({ organisationId });
            
            if (!api) {
                // If the organisation doesn't exist, create a new entry
                const newEntry = new dashboardSchema({
                    organisationId,
                    apiName
                });
                ...
                // If the organisation exists, update the apiName array
                const response = await dashboardSchema.findOneAndUpdate(
                    { organisationId },
                    { $set: { apiName } },
